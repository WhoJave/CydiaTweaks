...
function showContent() {
	...
	var creditElement = document.getElementById("reader-credits");
	if (creditElement) {
		if (creditElement.innerText != "undefined" && creditElement.innerText != "null")
...
var isReadModeActivated = false;
window.enterDarkReaderMode = function () {
	//remove any left over AMP attributes
	document.documentElement.removeAttribute('amp');
	var readerHtml = `READABILITY_HTML_PLACEHOLDER`;
	var documentClone = document.cloneNode(true);
	if (!readabilityResult)
		readabilityResult = new Readability(documentClone).parse();
	readerHtml = readerHtml.replace('%READER-TITLE%', readabilityResult.title);
	readerHtml = readerHtml.replace('%READER-TITLE%', readabilityResult.title);
	readerHtml = readerHtml.replace('%READER-CREDITS%', readabilityResult.credits ? readabilityResult.credits : readabilityResult.byline);
	readerHtml = readerHtml.replace('%READER-CONTENT%', readabilityResult.content);
	readerHtml = readerHtml.replace('%READER-URL%', readabilityResult.url);

	//var originalHtml = document.documentElement.outerHTML;
	console.log(readabilityResult);
	...
	showContent();
	updateImageMargins();
	document.documentElement.removeAttribute('deluminator_mode');
	isReadModeActivated = true;
	// document.getElementsByTagName('body')[0].outerHTML = article.content;
	// setTimeout(() => {
	//   document.write(originalHtml);
	// }, 2000);
}

window.isDarkReaderModeAvailable = function () {
	try {
		var documentClone = document.cloneNode(true);
		if (!readabilityResult)
			readabilityResult = new Readability(documentClone).parse();
		...
		if (!readabilityResult)
			return JSON.stringify({
				available: false,
				activated: isReadModeActivated
			});
		return JSON.stringify({
			available: readabilityResult.length > 600,
			activated: isReadModeActivated
		});
	} catch (err) {
		return JSON.stringify({
